...
import config  # <--- Importamos tus claves seguras
...
def analizar_con_ia(symbol, precio, rsi, vol):
    try:
        prompt = f"Actúa como trader senior. Analiza: {symbol}, Precio: {precio}, RSI: {rsi}, Vol: {vol}x. Dame un consejo corto (3 frases) para Sergio sobre si entrar o esperar. Sé directo."
        ...
        return "IA analizando... Intentá en un momento."
...
# --- UI ---
st.markdown("""<style>.stApp { background-color: #000; } .gold-header { color: #d4af37; text-align: center; font-size: 35px; border-bottom: 2px solid #d4af37; padding-bottom: 10px; margin-bottom: 20px;} .trade-card { background: #111; border-left: 5px solid #d4af37; border-radius: 10px; padding: 15px; margin-bottom: 5px; border: 1px solid #222; } .stButton>button { background-color: #d4af37 !important; color: black !important; font-weight: bold; width: 100%; } </style>""", unsafe_allow_html=True)
...
if df is not None:
    ...
    if upds:
        ...
        if "/analizar" in last_m:
            try:
                ...
                analisis = analizar_con_ia(target, row['price'], row['rsi'], row['vol'])
    ...
    with c1:
        ...
        for idx, row in df.sort_values(by=['decision', 'vol'], ascending=[False, False]).iterrows():
            color = "#00ff88" if "BUY" in row['decision'] else "#ff4b4b" if "SELL" in row['decision'] else "#d4af37" if "INFLOW" in row['decision'] else "#666"
            st.markdown(f"""<div class="trade-card"><b style="color:#d4af37;">{row['symbol']}</b> | <b style="color:{color};">{row['decision']}</b><br><span style="color:white; font-size:18px;">${row['price']:.4f}</span><br><small style="color:{color}; font-weight:bold;">{row['recomendacion']}</small></div>""", unsafe_allow_html=True)
            if st.button(f"IA ANALIZAR {row['symbol']}", key=f"ia_{row['symbol']}"):
